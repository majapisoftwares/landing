...
import { UnstyledAutocomplete, } from "../Autocomplete";
import { defaultLeadingClassName, InputIcon } from "../Input";
import { MagnifyingGlassIcon } from "@heroicons/react/20/solid";
export default function Spotlight({ open, onClose, placeholder = "Search...", query: defaultQuery = "", onChangeQuery, emptyText = "No item found.", ...props }) {
    ...
    return (<Transition.Root show={open} as={Fragment} afterLeave={() => setQuery("")} appear>
      <Dialog as="div" className="relative z-10" onClose={handleOnClose}>
        <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0" enterTo="opacity-100" leave="ease-in duration-200" leaveFrom="opacity-100" leaveTo="opacity-0">
          <div className={clsx("fixed inset-0 bg-zinc-500 !bg-opacity-25 transition-opacity dark:bg-zinc-800", "bg-white/50 backdrop-blur [@supports(backdrop-filter:blur(0))]:bg-white/30", "dark:bg-zinc-900/50 dark:[@supports(backdrop-filter:blur(0))]:bg-zinc-900/30")}/>
        </Transition.Child>
        ...
        <div className="fixed inset-0 z-10 overflow-y-auto p-4 sm:p-6 md:p-20">
          ...
          {query !== "" && filteredItems.length === 0 && (
            <p className="p-4 text-sm text-zinc-500">{emptyText}</p>
              ...
              <UnstyledAutocomplete {...props} query={query} onChangeQuery={setQuery} placeholder={placeholder} leadingClassName={defaultLeadingClassName} inputElementClassName="h-12 border-0 focus:ring-0" leading={<InputIcon>
                    <MagnifyingGlassIcon aria-hidden="true"/>
                  </InputIcon>} emptyTextClassName="p-4 text-sm text-zinc-500 dark:text-zinc-400" optionsClassName="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-zinc-800 dark:text-zinc-200" optionClassName={({ active }) => clsx("cursor-default select-none px-4 py-2", active && "bg-primary-600 text-white")} emptyText={emptyText}/>
            </Dialog.Panel>
          </Transition.Child>
        </div>
      </Dialog>
